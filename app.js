...
const CLOUD_KEY="kitoben_supabase_config_v1";
...
const DEFAULT_SUPABASE_CONFIG={
  url:"https://zkbydszptknesvhxmicl.supabase.co",
  anonKey:"sb_publishable_cO7-ogMOeezRBdzBKF4stw_DXGWSErQ"
};
const SHARED_STATE_ID="KITOBEN";
...
function cloudSetup(){
  ...
  openModal(`<h2>Supabase Cloud Setup</h2><p>Paste your Kitoben Supabase <b>Project URL</b> and <b>Publishable/anon key</b>. Never paste the service-role key into this website.</p>${field("Project URL","surl",c.url)}${field("Publishable / anon key","skey",c.anonKey)}<div class="toolbar"><button onclick="saveCloudSetup()">Save & Connect</button><button class="secondary" onclick="closeModal()">Cancel</button></div><p class="small">All authenticated Kitoben teachers can use the same shared school database. Create one Supabase Auth account for each teacher.</p>`);
}
...
function normalizeDbShape(){
  ...
  db.settings={school:"Kitoben Comprehensive School",motto:"Learn • Grow • Excel",term:"Term 3",year:"2026",phone:"",email:"",...(db.settings||{})};
...
function cloudStatusText(){
  ...
  if(cloudReady) return "☁️ Connected — shared Kitoben database is active.";
...
const KEY="kitoben_comprehensive_sms_v1";
...
function show(page){
  ...
  try{
    ...
    console.error("Kitoben page error:",err);
...
function Rankings(){
  ...
  document.getElementById("content").innerHTML=`<div class="card ranking-page">
    <div class="pagehero"><div><h2>Class Ranking & Subject Performance</h2><p>Rank learners by total marks across the active subjects.</p></div><span class="badge">Class Ranking</span></div>
...
function rankingSubjectLabel(subject){
  const map={
    "Mathematics":"Maths",
    "English":"Eng",
    "Kiswahili":"Kisw",
    ...
    "Environmental Activities":"Env.",
    "Creative Arts":"C.A.",
    "Creative Arts & Sports":"C.A.&S",
    "Religious Education":"R.E.",
    "Science & Technology":"Sci & Tech",
    "Integrated Science":"Int. Sci",
    "Agriculture":"Agric",
    "Social Studies":"S.Stud",
    "Pre-Technical Studies":"PreTec"
  };
...
function renderOverallRanking(){
  ...
  let subjects=subjectsForPeriod(c,ex.year,ex.term).slice(0,9);
  ...
  const missing=subjects.length<9?`<div class="small ranking-note no-print">This class currently has ${subjects.length} active subject${subjects.length===1?"":"s"}. The ranking table displays all active subjects, up to 9.</div>`:"";
  ...
  }).sort((a,b)=>b.avg-a.avg||a.subject.localeCompare(b.subject)).map((x,i)=>`<tr><td><b>${i+1}</b></td><td>${esc(x.subject)}</td><td>${x.n?x.avg.toFixed(2):"—"}</td><td>${x.n}</td></tr>`).join("");
  const subjectHeads=subjects.map(s=>`<th title="${esc(s)}">${esc(rankingSubjectLabel(s))}</th>`).join("");
  const learnerRows=learners.map(x=>`<tr>
    <td><b>${x.rank}</b></td>
    <td class="adm-cell">${esc(x.l.adm||"")}</td>
    <td class="name-cell"><b>${esc(x.l.name||"")}</b></td>
    ${x.performance.map(y=>`<td class="subject-mark">${y.mark===null?"—":Number(y.mark).toFixed(0)}</td>`).join("")}
    ${subjects.length<9?Array.from({length:9-subjects.length},()=>`<td class="subject-mark">—</td>`).join(""):""}
    <td class="total-cell"><b>${x.total.toFixed(0)}</b></td>
    <td class="grade-cell"><b>${grade(x.avg)}</b></td>
  </tr>`).join("");
  document.getElementById("rankout").innerHTML=`<div class="report ranking-report">
    <div class="schoolhead"><h1>${esc(db.settings.school||"Kitoben Comprehensive School")}</h1><p>${esc(c)} — CLASS PERFORMANCE & RANKING</p><p>${esc(ex.term||db.settings.term)}, ${esc(String(ex.year||db.settings.year))} • ${esc(ex.name||"")}</p></div>
    <section class="subject-ranking no-print"><h2>SUBJECT PERFORMANCE RANKING</h2><p class="small">The active subjects are ranked from highest to lowest class average.</p>${missing}<table><thead><tr><th>Position</th><th>Subject</th><th>Class Average</th><th>Learners with Marks</th></tr></thead><tbody>${subjectRows||`<tr><td colspan="4">No subject marks entered.</td></tr>`}</tbody></table></section>
    <section class="learner-ranking"><h2>CLASS RANKING</h2><table class="class-ranking-table"><thead><tr><th>Pos.</th><th>Adm.</th><th>Learner Name</th>${subjectHeads}${Array.from({length:9-subjects.length},()=>`<th>—</th>`).join("")}<th>Total / ${subjects.length*50}</th><th>Grade</th></tr></thead><tbody>${learnerRows||`<tr><td colspan="14">No valid marks for this class and assessment.</td></tr>`}</tbody></table></section>
    <div class="ranking-footer"><span>Subjects are marks out of 50. Position is based on average; total marks break ties.</span><span>Generated ${new Date().toLocaleDateString()}</span></div>
  </div>`;
}
function Attendance(){
...
function showReport(){
 ...
 let school=esc(db.settings.school||"Kitoben Comprehensive School"), term=esc(ex.term||db.settings.term), year=esc(String(ex.year||db.settings.year));
...
async function downloadSchoolFiles(){
  ...
  a.download="kitoben-comprehensive-school-files.json";
...
async function backup(){if(cloudReady)await cloudSave(true);let blob=new Blob([JSON.stringify(db,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="kitoben-comprehensive-school-backup.json";a.click();audit("Backup created")}
if(cloudConfigured()){
...
(function migrateV3(){
  ...
  if(!db.settings.school || /kitoben/i.test(db.settings.school)) db.settings.school="Kitoben Comprehensive School";
...
function schoolTitleForClass(c){return ecde.includes(c)?"KITOBEN ECDE SCHOOL":junior.includes(c)?"KITOBEN JUNIOR SCHOOL":"KITOBEN PRIMARY SCHOOL"}
function schoolTitleForSection(sec){return sec==="ECDE"?"KITOBEN ECDE SCHOOL":sec==="Junior"?"KITOBEN JUNIOR SCHOOL":"KITOBEN PRIMARY SCHOOL"}
