...
const CLOUD_KEY="tinet_kapkoi_supabase_config_v2";
...
const DEFAULT_SUPABASE_CONFIG={
  url:"https://bosvbweymitgmvkhkgsw.supabase.co",
  anonKey:"sb_publishable_h9kgwBH0-2NKMNnBisukkw_aKvrj9vK"
};
const SHARED_STATE_ID="TINETKAPKOI";
...
function normalizeDbShape(){
  ...
  db.settings={school:"Tinet-Kapkoi Comprehensive School",motto:"Learn • Grow • Excel",term:"Term 3",year:"2026",phone:"",email:"",...(db.settings||{})};
...
function renderOverallRanking(){
  ...
  document.getElementById("rankout").innerHTML=`<div class="report ranking-report">
    <div class="schoolhead"><h1>${esc(db.settings.school||"Tinet-Kapkoi Comprehensive School")}</h1><p>${esc(c)} — CLASS PERFORMANCE & RANKING</p><p>${esc(ex.term||db.settings.term)}, ${esc(String(ex.year||db.settings.year))} • ${esc(ex.name||"")}</p></div>
...
function showReport(){
 ...
 let school=esc(db.settings.school||"Tinet-Kapkoi Comprehensive School"), term=esc(ex.term||db.settings.term), year=esc(String(ex.year||db.settings.year));
...
(function migrateV3(){
  ...
  if(!db.settings.school || /tinet/i.test(db.settings.school)) db.settings.school="Tinet-Kapkoi Comprehensive School";
...
function schoolTitleForClass(c){return ecde.includes(c)?"TINET-KAPKOI ECDE SCHOOL":junior.includes(c)?"TINET-KAPKOI JUNIOR SCHOOL":"TINET-KAPKOI PRIMARY SCHOOL"}
function schoolTitleForSection(sec){return sec==="ECDE"?"TINET-KAPKOI ECDE SCHOOL":sec==="Junior"?"TINET-KAPKOI JUNIOR SCHOOL":"TINET-KAPKOI PRIMARY SCHOOL"}
